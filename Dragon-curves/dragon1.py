...
def main():
    ...
    dragon = []               # list for line segments
    
    h  = 2.0**(-p/2.0);  s = 0
    x1 = h * cos(p*pi/4.0)
    y1 = h * sin(p*pi/4.0)
    xt = 0.75*x1;  yt = 0.75*y1
    l  = Line(Point(xt, yt), Point(xt, yt))  # cannot initialize l in while loop
    dragon.append(l)
    n  = 1.0;  end = 2.0**p - 1.0
    while n < end:
        m = n
        ...
        l = Line(Point(xt, yt), Point(xa, ya))
        dragon.append(l)
        l = Line(Point(xa, ya), Point(xb, yb))
        dragon.append(l)
        xt = xb;  yt = yb
        x1 = x2;  y1 = y2;
        n  = n + 1.0
    for segment in dragon:
        segment.draw(win)
